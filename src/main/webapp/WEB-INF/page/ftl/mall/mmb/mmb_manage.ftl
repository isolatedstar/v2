...
<script type="text/javascript">
                ...
                if (data.mmbList.length > 0) {
                    for ( var i = 0; i < data.mmbList.length; i++) {
                        ...
                        if (data.mmbList[i].mmbStatus == 0) {
                            ...
                            content += "<a href='#' onclick=toManageMmbRelation('"+data.mmbList[i].id+"')>管理关系</a>&nbsp;";
                            content += "<a href='#' onclick=toManageMmbAdvert('"+data.mmbList[i].id+"')>推荐管理</a></td>";
                        } else {
    ...
    //进入推荐管理界面
    function toManageMmbAdvert(id){
        if(id!=""){
            //给隐藏框赋值
            $("#mmbId").val(id);
            //暂时省略校验部分，直接提交
            $.ajax({
                url:'${ctx}/mmbAdvert/queryMmbAdvert.do',
                type: "POST",
                dataType: "json",
                data:{"mmbId":id},
                success:function(data){
                    ...
                    if(data!=null&&data.length>0){
                        if(data!=""&&data.length>0){
                            for(var i=0;i<data.length;i++){
                                var trHtml="<tr id='mmbRelaTable"+i+"' ><td><input type='checkbox' name='advertMmb' value='"+data[i].id+"'/></td><td>"+data[i].advertFname+"</td><td>"+data[i].advertHomePage+"</td></tr>";
                                addTr('mmbAdvertTable', -1, trHtml);
                            }

                        }
                        //打开管理会员关系模态框
                        $("#manageAdvert").modal('show');
                    }
                    //如果失败
                    else{
                        //打开管理会员关系模态框
    ...
    //全选反选会员推荐
    function checkAllAdvert(obj){
        $("#mmbAdvertTable tr td [name = advertMmb]:checkbox").each(function(i,thisElement){
            if($(obj).prop("checked")==true){
                $(thisElement).prop("checked",'true');
            }else{
                $(thisElement).prop("checked",false);
    ...
    function subAdvertMmbs(){
        ...
        //提交创建推荐关系
        $.ajax({
            url : '${ctx}/mmbAdvert/createMmbAdverts.do',// 跳转到 action
            data : {
                mmbIds : stringsId,
                mmbId:$("#mmbId1").val()
            },
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            if(data!=null&&data.length>0){
                                ...
                                //先清空table中的数据
                                $("#mmbAdvertTable  tr:not(:first)").remove();
                                if(data!=""&&data.length>0){
                                    for(var i=0;i<data.length;i++){
                                        var trHtml="<tr><td><input type='checkbox' name='advertMmb' value='"+data[i].id+"'/></td><td>"+data[i].advertFname+"</td><td>"+data[i].advertHomePage+"</td></tr>";
                                        addTr('mmbAdvertTable', -1, trHtml);
                                    }
                                }
                            }
    ...
    //批量删除推荐会员
    function deleteMmbAdvert(){
        var checkedMmbs = $("#mmbAdvertTable tr td input[name=advertMmb]:checked");
        if(checkedMmbs.length==0){
            alert("请选择需要删除的推荐会员！");
            return false;
        }
        ...
        //提交批量删除信息
        $.ajax({
            url : '${ctx}/mmbAdvert/batchDeleteAdvert.do',// 跳转到 action
            data : {
                ids : stringsId
            },
            type : 'POST',
                    ...
                    $.ajax({
                        ...
                        success:function(data1){
                            ...
                            if(data!=null&&data.length>0){
                                ...
                                if(data!=""&&data.length>0){
                                    for(var i=0;i<data.length;i++){
                                        var trHtml="<tr><td><input type='checkbox' name='relaMmb' value='"+data[i].id+"'/></td><td>"+data[i].advertFname+"</td><td>"+data[i].advertHomePage+"</td></tr>";
                                        addTr('mmbAdvertTable', -1, trHtml);
                                    }
                                }
                            }else if(data.length == 0){
        ...
        $("#mmbAdvertAll").attr("checked",false);
    }


    //推荐管理 点击添加
    function addMmbAdvert(){
        //将当前选中的会员id传入
        $("#mmbId1").val($("#mmbId").val());
        $("#relatype").val("advert");//表示 为推荐关系进入
...
<!--管理会员推荐-->
<!--弹出层-->
<div class="modal fade" id="manageAdvert" role="dialog" aria-labelledby="gridSystemModalLabel" data-backdrop="static">
    <div class="modal-dialog m-tanchu-box" role="document">
        <div class="container-fluid" style=" margin-top:15px;">
            <div class="row-fluid">
                <!-- col-sm-12 -->
                <div class="col-sm-12 ">
                    <div class="panel panel-default article-bj">
                        ...
                        <div class="table-responsive panel-table-body ">
                            <table class="table table-striped table-hover " id="mmbAdvertTable">
                                <thead>
                                <tr>
                                    <th><input type="checkbox" id="mmbAdvertAll" onclick="checkAllAdvert(this)"></th>
                                    <th>全称</th>
                                    <th>主页</th>
                                </tr>
                                </thead>
                        ...
                        <footer class="panel-footer text-right bg-light lter">
                            <button class="btn btn-success btn-s-xs" type="button" onclick="addMmbAdvert();">添加</button>
                            <button class="btn btn-info btn-s-xs" type="button" onclick="deleteMmbAdvert();">删除</button>
                            <button class="btn btn-warning btn-s-xs" type="submit" data-dismiss="modal">退出</button>
                        </footer>
                    </div>
                </div>
                ...
                <div class="col-sm-12 ">
                    <div class="panel panel-default article-bj">
                        ...
                        <footer class="panel-footer text-right bg-light lter">
                            ...
                            <button class="btn btn-warning btn-s-xs" type="button" id="subAdvertMmbs" onclick="subAdvertMmbs()">确定</button>
                            <button class="btn btn-warning btn-s-xs" type="button" id="submitMmbs" onclick="submitMmbs()">确定</button>
