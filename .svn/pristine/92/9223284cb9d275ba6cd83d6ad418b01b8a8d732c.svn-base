...
<body>
		...
		<div class="row-fluid">
			...
			<div class="col-sm-12 ">
				<div class="panel panel-default article-bj">
					<div class="panel-heading box-shodm">收退款</div>
					<form id="queryGetRefundMoneyForm">
						<div class="row wrapper form-margin">
							...
							<input type="button" class="btn btn-info btn-s-md btn-default float-right cx-btn-margin" value="查询" style="height:35px;width:65px" onclick="queryGetRefundMoney();" id="queryGetRefundMoneyButton"/>
						</div>
						<div class="table-responsive panel-table-body ">
							<table class="table table-striped table-hover" id="tb_getRefundMoney" ></table>
						</div>
					</form>
					<footer class="panel-footer text-right bg-light lter">
						<button class="btn btn-warning btn-s-xs" type="button" onclick="getRefundMoney();">收款</button>
					</footer>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$("#queryGetRefundMoneyForm").validationEngine('attach',{
				scroll:false,
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$("#tb_getRefundMoney").bootstrapTable({
					url : "<%=basePath%>order/queryGetRefundMoney.do",
					method : "post",
					dataType : "json",
		...
		function queryGetRefundMoney() {
			var pageSize = $("#tb_getRefundMoney").bootstrapTable("getOptions").pageSize === $(
					"#tb_getRefundMoney").bootstrapTable("getOptions")
					.formatAllRows() ? $("#tb_getRefundMoney").bootstrapTable(
					"getOptions").totalRows : $("#tb_getRefundMoney")
					.bootstrapTable("getOptions").pageSize;
			$.ajax({
				url : "<%=basePath%>order/queryGetRefundMoney.do?pageNo=0&pageSize="+ pageSize,
				data : {
					param : JSON.stringify($("#queryGetRefundMoneyForm")
							.serializeJson())
				},
				type : "POST",
				cache : false,
				...
				success : function(data) {
					$("#tb_getRefundMoney").bootstrapTable("load", data);
				},
				error : function() {
					alert("异常！");
				}
		...
		function getRefundMoney() {
			var allData = $("#tb_getRefundMoney").bootstrapTable("getData");
			var dataObj = $("#tb_getRefundMoney").bootstrapTable("getSelections");
...
// 			});
			if(!$("#queryGetRefundMoneyForm").validationEngine("validate")){
				return false;
			}
			$.ajax({
				url : "<%=basePath%>order/getRefundMoney.do",
				data : {
				...
				success : function(data) {
					alert(data.msg);
					$("#tb_getRefundMoney").bootstrapTable("refresh");
				},
				error : function() {
					alert(data.msg);
				}
