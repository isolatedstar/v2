...
orderDetail.prototype.init=function(){
	...
	html.push('<div class="input-group">');
	html.push('<div class="input-group-btn">');
	html.push('<h5 class="h5-margin" id="o_t_time"></h5>');
	html.push('</div>');
	html.push('</div>');
	html.push('</div>');
	html.push('<div class="col-md-4">');
	html.push('<div class="input-group">');
	html.push('<div class="input-group-btn">');
	html.push('<h5 class="h5-margin" id="o_t_money"></h5>');
	html.push('</div>');
	html.push('</div>');
	html.push('</div>');
	html.push('</div>');
...
orderDetail.prototype.queryOrderDetail=function(id){
	...
	$.ajax({
		...
		success : function(data) {
			...
			$("#o_t_flow").text(_flow);
			$("#o_t_time").text("签约时间:"+$.changeDate(data.ordertitle.lockTime));
			$("#o_t_money").text("总金额:"+data.ordertitle.totalMoney+"元");
