   ...
   $(function(){
		...
		li.click(function(){  
		    ...
		    // var change = $("#selecter_basic").val();
			 ...
			 window.location = "${ctx}/assureController/toAssureNoFilter1.do?page="+pageNo+"&payerName="+payerName+"&payeeName="+payeeName;
		})	
	})
<#--  分页over   -->	
...
<#--  查询  -->
 function getMassage(){
 	...
 	window.location = "${ctx}/assureController/toAssureNoFilter1.do?payerName="+payerName+"&payeeName="+payeeName;//+"&type="+type;
 }
 <#--  查询结束   -->
...
function assureRegister(){
	...
	$.ajax({
		...
		success:function(data){
			...
			$("#myModalTS").modal("hide");
			alert(data);
			window.location = "${ctx}/assureController/toAssureNoFilter1.do";
		}
	})
}
<#-- 进行登记操作结束   -->
		...
		<div class="fs">
			...
			<div class="panel panel-default article-bj">
			  ...
			  <div class="row wrapper form-margin">
				<form id="formId" action ="${ctx}/assureController/toAssureNoFilter1.do" method="post">
				  <div class="col-md-3">
