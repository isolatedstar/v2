  ...
  <script type="text/javascript">
  	...
  	function showDetail(){
	    ...
	    for(var j=1;j<len;len--){
	 	...
	 	$.ajax({
			url:'${ctx}/refundMgController/selectRepaymenDbList.do',
			dataType:"JSON",
			type:"POST",
			data:{"guaId":guaId, "refundCount":refundCount},
			success:function(data){
			    data = eval('('+data+')');
			    var restr = data.restr;
			    var err = data.err;
			    var fin = data.fin;
			    $("#hidGuaId").val(data.guaId);
  	...
  	function executeMessage(){
	 	$.ajax({
			url:'${ctx}/refundMgController/disposeGuaranteeRefund.do',
			dataType:"JSON",
			type:"POST",
			data:{"guaId":$("#hidGuaId").val(), "lockFinJson":$("#lockFinJson").val(), "lockGuaJson":$("#lockGuaJson").val(), "repaymentJson":$("#repaymentJson").val()},
			success:function(data){
				alert(data);
  ...
  <div class="container-fluid">
    <div class="row-fluid">
      ...
      <div class="panel panel-default article-bj">
        <div class="panel-heading box-shodm">担保还款</div>
        <div class="row wrapper form-margin">
          <form name="selectForm" id="selectForm" method="post">
            <input type="hidden" name="hidGuaId" id="hidGuaId"/>
            ...
            <div class="row wrapper form-margin">
             <div class="col-md-3">
              <div class="input-group">
                ...
                <input type="text" placeholder="" class="form-control" name="guaId" id="guaId"></div>
              </div>
              <div class="col-md-3">
              <div class="input-group">
                <div class="input-group-btn">
