...
public class PagePlugin implements Interceptor {
	 ...
	 * @return
	 */
	private String generatePageSql(String sql,@SuppressWarnings("rawtypes") Page page){
		if(page!=null && dialect!=null && dialect.trim().length()>0){
			StringBuffer pageSql = new StringBuffer();
			if("mysql".equals(dialect)){
				pageSql.append(sql);
				pageSql.append(" limit "+page.getBeginIndex()+","+page.getPageSize());
			}else if("oracle".equals(dialect)){
				pageSql.append("select * from (select tmp_tb.*,ROWNUM row_id from (");
				pageSql.append(sql);
				pageSql.append(") as tmp_tb where ROWNUM<=");
				pageSql.append(page.getBeginIndex()+page.getPageSize());
				pageSql.append(") where row_id>");
				pageSql.append(page.getBeginIndex());
			}
