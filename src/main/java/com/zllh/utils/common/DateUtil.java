...
public class DateUtil {
	...
	public static final String codeDateFormat = "yyyyMMdd";
	...
	public static String getNowDateCode() {
		SimpleDateFormat sdf = new SimpleDateFormat(codeDateFormat);
		Calendar cal = Calendar.getInstance();
	...
	public static String getTimeBeforeMinutes(String dateStr, int minute){
		Date date = DateUtil.parseDate(dateStr, normalTimeFormat);
		Calendar cNow = Calendar.getInstance();
		cNow.setTime(date);
		cNow.add(Calendar.MINUTE, minute);
		return formatDate(cNow.getTime(), normalTimeFormat);
	 ...
	 * 说明：获取某日期前n小时的日期
	 */
	public static String getBackHourFromDate(String date, int hours){

		String newDate = "";

		Calendar calendar = Calendar.getInstance();
		calendar.setTime(DateUtil.parseDate(date, normalTimeFormat));
		calendar.add(Calendar.HOUR, -hours);
		newDate = formatDate(calendar.getTime(), normalTimeFormat);;
	...
	/**
	 * 比较两个时间的大小，date1>date2 返回1；date1<date2 返回-1；date1=date2 返回0
	 * @param Time1
	 * @param Time2
	 * @return
	 */
	public static int compareDate(String date1, String date2) {
		try {
			Date dt1 = parseDate(date1, normalDateFormat);
			Date dt2 = parseDate(date2, normalDateFormat);
			if (dt1.getTime() > dt2.getTime()) {
				return 1;
			} else if (dt1.getTime() < dt2.getTime()) {
				return -1;
	...
	public static String getBeforDay(int n){
		return DateUtil.formatDate(DateUtil.rollDate(-n), normalDateFormat);
	}

	
	/**
	 * 区分 日、周、月，日的不变，周的根据startTime获取周一时间，根据endTime获取周日时间，月的根据startTime获取1号，根据endTime获取最后一天
	 * @param searchType 
	 * @param startTime
	 * @param endTime
	 * @return
	 */
	public static String[] getDateTimes(String searchType, String startTime, String endTime){
		...
		try {
			if("day".equals(searchType)){
				...
				times[1] = getMonthLastDay(endTimes[0], endTimes[1]).substring(0, 10);
				times[0] = startTimes[0] +  "-" + startTimes[1] + "-" + "01";
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		System.out.println("\u6210\u90fd\u5e02\u6b66\u4faf\u533a");
	}
	
	/**
	 * @Title: formatDiff 
	 * @author JW
	 * @Description: 根据时间差秒数转换为x天x小时x分
	 * @param diff
	 * @return String
	 * @throws
	 */
