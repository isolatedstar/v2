...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。（单张图片限定大小为'+(para.fileSize/1024)/1024+'M。）</div>';
		            html += '				<div class="btns">';
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张图片，共0B。（单张图片限定大小为'+(para.fileSize/1024)/1024+'M。）</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$("#status_info").html("选中"+num+"张文件，共"+size+"。（单张图片限定大小为"+(para.fileSize/1024)/1024+"M。）" );
			};
