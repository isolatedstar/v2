			...
			<div class="col-sm-12 ">
				<div class="panel panel-default article-bj">
					...
					<footer class="panel-footer text-right bg-light lter">
						<button class="btn btn-warning btn-s-xs" type="button" onclick="lockOrder();">锁定</button>
						<button class="btn btn-danger btn-s-xs" type="button" onclick="abolishOrder();">作废</button>
					</footer>
				</div>
			</div>
		</div>
...
<div class="modal fade" id="orderDetail" role="dialog" aria-labelledby="gridSystemModalLabel" data-backdrop="static"></div>
	<script type="text/javascript">
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$("#tb_pendingOrder").bootstrapTable({
					url : "<%=basePath%>order/queryPendingOrder.do",
					method : "post",
					dataType : "json",
					...
					columns : [
							...
							},
							{
								...
								sortable : false,
								formatter : function(value,row, index) {
									return '<a href="#" data-toggle="modal" onclick="showEditOrder(\''+row.id+'\');">编辑</a>';
								}
							}]
					});
			};
		...
		function queryPendingOrder() {
			...
			$.ajax({
				url : "<%=basePath%>order/queryPendingOrder.do?pageNo=0&pageSize="+ pageSize,
				data : {
					name : $("#name").val(),
					orderId : $("#orderId").val(),
					status : $("#status option:selected").val(),
		...
		function showEditOrder(id){
			...
			//订单详细代码，复用需拷贝
			editOrder.queryOrderDetail(id);
		}
		function lockOrder() {
			$("input[name='btSelectItem']").attr("class","validate[minCheckbox[1]] checkbox");
			var dataObj = $("#tb_pendingOrder").bootstrapTable("getSelections");
			if(!$("#queryPendingOrderForm").validationEngine("validate")){
				return false;
			}
// 			if (dataObj.length == 0) {
// 				alert("请选择订单！");
...
// 			}
			if(confirm("确认要锁定订单吗？")){
				$.ajax({
					url : "<%=basePath%>order/lockOrder.do",
					data : {
					...
					success : function(data) {
						alert(data.msg);
						$("#tb_pendingOrder").bootstrapTable("refresh");
					},
					error : function() {
						alert(data.msg);
					}
		...
		function abolishOrder() {
			...
			if(confirm("确认要作废订单吗？")){
				$.ajax({
					url : "<%=basePath%>order/abolishOrder.do",
					data : {
						param : JSON.stringify(dataObj)
					},
					type : "POST",
